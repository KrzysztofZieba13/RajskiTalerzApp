import { Link } from "react-router-dom";
import styles from "./HeaderNavList.module.css";
import { useSelector } from "react-redux";
import { selectCartLength } from "../features/cart/cartSlice";
import { useGetMyOrdersQuery } from "../services/orders";
import { useGetMyBookingsQuery } from "../services/bookings";

function HeaderNavList({
  isUser,
  isActive,
  user,
  handleHover,
  handleCart,
  handleNotification,
  handleMobileNav,
}) {
  const { data: orders, isLoading: isLoadingOrders } = useGetMyOrdersQuery(
    "W drodze,Gotowe,W realizacji&isVisibleNotification=true"
  );
  const { data: bookings, isLoading: isLoadingBookings } =
    useGetMyBookingsQuery(true);

  const cartLength = useSelector(selectCartLength);

  if (isLoadingOrders || isLoadingBookings) return;

  let notificationsQuantity;
  if (orders && bookings)
    notificationsQuantity = orders.results + bookings.length;

  let navTo;
  if (user)
    switch (user.role) {
      case "waiter":
        navTo = "waiter/orders";
        break;
      case "cook":
        navTo = "kitchen";
        break;
      case "deliveryGuy":
        navTo = "delivery";
        break;
      case "admin":
        navTo = "admin/charts";
        break;
    }

  return (
    <ul className={styles.headerNavList}>
      {isUser && isUser.role !== "client" && (
        <li
          style={isActive(1)}
          onMouseEnter={() => handleHover(1)}
          onMouseLeave={() => handleHover(null)}
        >
          <Link to={navTo}>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 24 24"
              strokeWidth={1.5}
              stroke="currentColor"
              className="size-6"
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
                d="M9.594 3.94c.09-.542.56-.94 1.11-.94h2.593c.55 0 1.02.398 1.11.94l.213 1.281c.063.374.313.686.645.87.074.04.147.083.22.127.325.196.72.257 1.075.124l1.217-.456a1.125 1.125 0 0 1 1.37.49l1.296 2.247a1.125 1.125 0 0 1-.26 1.431l-1.003.827c-.293.241-.438.613-.43.992a7.723 7.723 0 0 1 0 .255c-.008.378.137.75.43.991l1.004.827c.424.35.534.955.26 1.43l-1.298 2.247a1.125 1.125 0 0 1-1.369.491l-1.217-.456c-.355-.133-.75-.072-1.076.124a6.47 6.47 0 0 1-.22.128c-.331.183-.581.495-.644.869l-.213 1.281c-.09.543-.56.94-1.11.94h-2.594c-.55 0-1.019-.398-1.11-.94l-.213-1.281c-.062-.374-.312-.686-.644-.87a6.52 6.52 0 0 1-.22-.127c-.325-.196-.72-.257-1.076-.124l-1.217.456a1.125 1.125 0 0 1-1.369-.49l-1.297-2.247a1.125 1.125 0 0 1 .26-1.431l1.004-.827c.292-.24.437-.613.43-.991a6.932 6.932 0 0 1 0-.255c.007-.38-.138-.751-.43-.992l-1.004-.827a1.125 1.125 0 0 1-.26-1.43l1.297-2.247a1.125 1.125 0 0 1 1.37-.491l1.216.456c.356.133.751.072 1.076-.124.072-.044.146-.086.22-.128.332-.183.582-.495.644-.869l.214-1.28Z"
              />
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
                d="M15 12a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z"
              />
            </svg>
          </Link>
        </li>
      )}
      {isUser && (
        <li
          className={styles.notificationBtn}
          style={isActive(2)}
          onMouseEnter={() => handleHover(2)}
          onMouseLeave={() => handleHover(null)}
        >
          <Link onClick={() => handleNotification(true)}>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 24 24"
              strokeWidth={1.5}
              stroke="currentColor"
              className="size-6"
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
                d="M14.857 17.082a23.848 23.848 0 0 0 5.454-1.31A8.967 8.967 0 0 1 18 9.75V9A6 6 0 0 0 6 9v.75a8.967 8.967 0 0 1-2.312 6.022c1.733.64 3.56 1.085 5.455 1.31m5.714 0a24.255 24.255 0 0 1-5.714 0m5.714 0a3 3 0 1 1-5.714 0"
              />
            </svg>
          </Link>
          {notificationsQuantity > 0 && (
            <span className={styles.displayQuantity}>
              {notificationsQuantity}
            </span>
          )}
        </li>
      )}
      <div>
        <li
          className={styles.cartBtn}
          style={isActive(3)}
          onMouseEnter={() => handleHover(3)}
          onMouseLeave={() => handleHover(null)}
        >
          <Link onClick={() => handleCart(true)}>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 24 24"
              strokeWidth={1.5}
              stroke="currentColor"
              className="size-6"
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
                d="M15.75 10.5V6a3.75 3.75 0 1 0-7.5 0v4.5m11.356-1.993 1.263 12c.07.665-.45 1.243-1.119 1.243H4.25a1.125 1.125 0 0 1-1.12-1.243l1.264-12A1.125 1.125 0 0 1 5.513 7.5h12.974c.576 0 1.059.435 1.119 1.007ZM8.625 10.5a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Zm7.5 0a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Z"
              />
            </svg>
          </Link>
          {cartLength > 0 && (
            <span className={styles.displayQuantity}>{cartLength}</span>
          )}
        </li>
      </div>
      <div>
        <li
          style={isActive(4)}
          onMouseEnter={() => handleHover(4)}
          onMouseLeave={() => handleHover(null)}
        >
          <Link onClick={() => handleMobileNav(true)}>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 24 24"
              strokeWidth={1.5}
              stroke="currentColor"
              className="size-6"
              style={isActive(4)}
              onMouseEnter={() => handleHover(4)}
              onMouseLeave={() => handleHover(null)}
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
                d="M3.75 6.75h16.5M3.75 12h16.5m-16.5 5.25h16.5"
              />
            </svg>
          </Link>
        </li>
      </div>
    </ul>
  );
}

export default HeaderNavList;
